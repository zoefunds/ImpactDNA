#!/usr/bin/env node
/**
 * Deploys ImpactDnaEscrow.sol to Base Sepolia.
 *
...
function compile() {
  const sourcePath = path.join(__dirname, "ImpactDnaEscrow.sol");
  const source = fs.readFileSync(sourcePath, "utf8");
  const input = {
    language: "Solidity",
    sources: { "ImpactDnaEscrow.sol": { content: source } },
    settings: {
  ...
  const contract = output.contracts["ImpactDnaEscrow.sol"]["ImpactDnaEscrow"];
...
async function main() {
  ...
  console.log("ImpactDnaEscrow deployed at:", address);
  console.log("\nSet these in backend/.env:");
  console.log(`IMPACT_DNA_ESCROW_ADDRESS=${address}`);
